...
namespace CurencyHire.Core
{
    public class Graph
    {
        ...
        public void PrintMatrix(ref double?[,] matrix, string[] labels, int count)
        {
            Console.Write("       ");
        ...
        public bool PrintPath(ref double?[,] graph, string[] labels, string src, string dest, ref double?[,] rateGraph, out double outRate)
        {
            ...
            if (paths.Count > 0)
            {
                ...
                for (int i = 0; i < paths.Count - 1; i++)
                {
                    ...
                    // var t = this.AllNodes.Select(a => a.Edges.Select(b => b.Rate));
                    //graph.AllNodes.Select(s => s.Name)

                    Console.Write($"{labels[paths[i]]} [{length}] -> [{rate}] ");
                }
                Console.WriteLine($"{labels[destination]} (Distance {path_length})");
                Console.WriteLine($"{labels[destination]} (Total Rate {rate_total})");
